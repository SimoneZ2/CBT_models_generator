...
def _normalize_gold_model(gold: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalizza le chiavi del gold per compatibilità con compare_models.
    """
    g = dict(gold)
...
def _normalize_pred_model(pred: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalizza il pred (che può avere stringhe invece di liste).
    Mantiene lo schema compatibile con compare_models.
    """
    p = dict(pred)

    # fields che in pipeline erano list
    for k in ["helpless_belief", "unlovable_belief", "worthless_belief", "emotion", "type", "coping_strategies"]:
        if k in p:
            # coping_strategies in alcuni tuoi esempi è stringa: lo portiamo a lista
            p[k] = _safe_list(p.get(k))
...
def _extract_nli_contradiction_overall(nli_out: Dict[str, Any]) -> Optional[float]:
    """
    Media per-field di (contr_g2p + contr_p2g)/2, poi media sui field.
    """
...
def _patient_id_to_source_id(patient_id: str, suffix: str) -> str:
...
def run(
    *,
    ...
    predicted_models = _load_json(predicted_path)
    if not isinstance(predicted_models, list):
        raise ValueError(f"Predicted file must be a JSON list. Got: {type(predicted_models)}")

    gold_models = _load_json(gold_path)
    if not isinstance(gold_models, list):
        raise ValueError(f"Gold file must be a JSON list. Got: {type(gold_models)}")
    ...
    for pred in predicted_models:
        ...
        patient_id = str(pred.get("id", "") or "").strip()
        source_id = _patient_id_to_source_id(patient_id, suffix=suffix)
        if not source_id:
            if verbose:
                logger.info("[SKIP] predicted item has no id")
            continue
        ...
        pred_norm = _normalize_pred_model(pred)

        # salva “pred” normalizzato in generated_cbts/<source_id>.json
        pred_path = os.path.join(generated_dir, f"{source_id}.json")
        _save_json(pred_path, pred_norm)

        # ricarica per coerenza (stesso pattern dei tuoi script precedenti)
        predicted_loaded = _load_json(pred_path)

        if verbose:
            logger.info(f"SOURCE_ID: {source_id} (patient_id={patient_id})")
            logger.info(f"GOLD_ID: {gold_norm.get('id')}")
            logger.info(f"PRED_NAME: {pred_norm.get('name', '')}")
    ...
    logger.info(f"\nCompleted. Processed {count} predicted model(s).")
    ...
    final_report["mapping"] = {"patient_id_to_source_id": f"id + '{suffix}'"}
...
def main() -> None:
    ...
    ap.add_argument("--predicted", default="predicted_deepseek_sit1.json", type=str,  help="Path JSON con lista di CBT predetti (Patient 1,2,...)")
    ap.add_argument("--gold", default="Patient_Psi_CM_Dataset.json", type=str, help="Path al file CBT gold")
    ap.add_argument("--out_dir", default="output_deepseek_sit1", type=str, help="Cartella output")
    ap.add_argument("--max_n", type=int, default=None, help="Massimo numero di modelli da valutare (default: tutti)")
    ap.add_argument(
        "--suffix",
        type=str,
        default="-2",
        help="Suffix per mappare patient_id -> source_id (default: -2, es: 1 -> 1-2)",
    )
