...
# ... get_gold_major, get_gold_fine, load_model, metriche invariati ...
...
def _pick_join_key(example: Dict[str, Any], candidates: List[str]) -> Optional[str]:
    """Return the first key in candidates that exists and is non-empty in example."""
    for k in candidates:
        v = example.get(k, None)
        if v is None:
            continue
        # allow int/str ids; reject empty strings
...
def _build_index(rows: List[Dict[str, Any]], key: str) -> Dict[str, Dict[str, Any]]:
    """Index rows by key (stringified). If duplicates exist, keep the first."""
    idx = {}
    for r in rows:
        v = r.get(key, None)
        if v is None:
...
def main():
    ...
    ap.add_argument("--data", default="Patient_Psi_CM_Dataset.json", help="GOLD dataset (.jsonl o .json)")
    ap.add_argument("--fmt", choices=["jsonl", "json"], default="json")

    ap.add_argument(
        "--pred_data",
        default="outputs_pipeline_current_sit1_2/merged_cbts.json",
        help="Dataset con i campi testuali per le predizioni (situation/intermediate_belief/auto_thought)"
    )
    ap.add_argument("--pred_fmt", choices=["jsonl", "json"], default="json")

    ap.add_argument("--major_dir", default=MAJOR_DIR)
    ap.add_argument("--fine_dir", default=FINE_DIR)
    ap.add_argument("--major_threshold", type=float, default=0.50)
    ap.add_argument("--major_threshold_worthless", type=float, default=0.6,
                    help="Override threshold only for 'worthless'. If None uses --major_threshold.")
    ...
    gold_data = load_dataset(args.data, args.fmt)
    print(f"[INFO] loaded GOLD {len(gold_data)} examples from {args.data}")

    pred_data = load_dataset(args.pred_data, args.pred_fmt)
    print(f"[INFO] loaded PRED {len(pred_data)} examples from {args.pred_data}")

    candidate_keys = ["id", "example_id", "uid", "guid", "case_id", "dialogue_id", "sample_id", "item_id"]

    gold_key = _pick_join_key(gold_data[0], candidate_keys) if gold_data else None
    pred_key = _pick_join_key(pred_data[0], candidate_keys) if pred_data else None

    join_key = None
    if gold_key and pred_key and gold_key == pred_key:
        join_key = gold_key
    elif gold_key and pred_key:
        for k in candidate_keys:
            if (k in gold_data[0]) and (k in pred_data[0]):
                v1 = gold_data[0].get(k)
                v2 = pred_data[0].get(k)
                if v1 is not None and v2 is not None:
                    join_key = k
                    break

    pred_index = _build_index(pred_data, join_key) if join_key else None

    if join_key:
        print(f"[INFO] aligning GOLD<->PRED by key='{join_key}'")
    else:
        if len(gold_data) == len(pred_data):
            print("[WARN] no common id key found -> aligning by index (same length).")
        else:
            print("[WARN] no common id key found and lengths differ -> will align by id if possible later, else skip unmatched.")

    texts: List[str] = []
    aligned_gold: List[Dict[str, Any]] = []  
    aligned_pred: List[Dict[str, Any]] = []  
    skipped = 0
    for i, gex in enumerate(gold_data):
        pex = None
        if join_key:
            gid = gex.get(join_key, None)
            if gid is not None:
                pex = pred_index.get(str(gid))
        else:
            if i < len(pred_data):
                pex = pred_data[i]
        ...
        aligned_gold.append(gex)
        aligned_pred.append(pex)
        texts.append(build_input_text(pex))


    if skipped:
        print(f"[WARN] skipped {skipped} GOLD examples because no matching PRED example was found.")
    print(f"[INFO] evaluating on {len(aligned_gold)} aligned examples")

    ids_eval = [str(ex.get(join_key)) for ex in aligned_gold]
    Path("ids_eval.txt").write_text("\n".join(ids_eval), encoding="utf-8")
    print("[INFO] wrote ids_eval.txt")
    ...
    for idx, gex in enumerate(aligned_gold):
        gold_major = get_gold_major(gex)
        gold_fine_raw = get_gold_fine(gex)
    ...
    report = {
        ...
        "n_examples_gold": len(gold_data),
        "n_examples_pred": len(pred_data),
        "n_examples_aligned": len(aligned_gold),
